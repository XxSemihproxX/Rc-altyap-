...
if(!mesaj) {
	message.reply("Lütfen Herhangi Bir Radyo Numarası Seçiniz.\n\n__Radyo Kanalları__\n\n**[1] - JoyTürk**\n**[2] - SlowTürk**\n**[3] - Fenomen FM**\n**[4] - Metro**\n**[5] - Number1**\n**[6] - PowerTürk**\n\n\n\nÖrnek = **.radyo 2**\nGelişmiş Türk Radyo Sistemi")
}		
    if (!message.member.voiceChannel) return message.reply("Radyo Komutunu Kullanabilmek İçin Lütfen Sesli Bir Kanala Giriş Yapınız. <a:hayir:600271095635116062>");

		    if (mesaj === "1") {
        if (message.member.voiceChannel.join()
        .then(connection => {
            const dispatcher = connection.playStream(joyturk);
            return message.channel.send("**JoyTürk** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    };
        ...
        .then(connection => {
            ...
            return message.channel.send("**SlowTürk** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    };
    // 2 Fenomen
    if (mesaj === "3") {
        ...
        .then(connection => {
            ...
            return message.channel.send("**Fenomen** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    };
    // 6 Metro
    if(mesaj === "4") {
        ...
        .then(connection => {
            ...
            return message.channel.send("**Metro** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    };
    // 8 Number1
    if(mesaj === "5") {
        ...
        .then(connection => {
            ...
            return message.channel.send("**Number1** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    };    
    // 14
    if(mesaj === "6") {
        ...
        .then(connection => {
            ...
            return message.channel.send("**PowerTürk** FM Oynatılıyor. <a:onay:600270660031479821>");
        }));
        return;
    } 
